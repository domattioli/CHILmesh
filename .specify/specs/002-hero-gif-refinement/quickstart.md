...
| **SC-006** | GIF ≤ 4.2 MB and animates in the GitHub README. | `ls -l docs/gallery/readme_pipeline_annulus.gif` → ≤ 4.2 MB (post-process 256c+optimize projects ~2.1–2.3 MB). Confirm dims 864×396 (`python -c "from PIL import Image; print(Image.open('docs/gallery/readme_pipeline_annulus.gif').size)"`). Manual: view on the pushed `development` branch's README. |
