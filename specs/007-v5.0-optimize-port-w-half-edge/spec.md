...
- **Mixed-element padded triangles**: The current adjacency model uses a padded-quad convention (vertex 0 in slot 3 marks a triangle in a 4-column array). Half-edge must preserve this without storing fake edges for the padding slot, otherwise `_elem_type` regresses (B3 from v0.1.1). Specifically: half-edge construction MUST check element type per row and skip the padded slot for triangles, reusing the existing `_elem_type` vectorized mask.
...
- **Degenerate elements** (collinear quad vertices): Zero-area triangles / zero-length edges (e.g., the quad angle test from B5) crash when half-edge `next_idx` becomes ambiguous. Solution: half-edge construction MUST run `_ensure_ccw_orientation()` (existing B4 fix) BEFORE assigning `next_idx` pointers. Include regression test using the synthetic degenerate quad fixture from v0.1.1 (`tests/test_interior_angles.py::test_quad_angles_no_nan_on_degenerate_quad`).
- **Environment variable mis-set**: If `CHILMESH_TOPOLOGY_BACKEND` is set to an unknown value (e.g., typo "halfedage"), the system MUST raise a clear `ValueError` at mesh construction — not silently fall back to EdgeMap and confuse the benchmark. Additionally, a pytest fixture MUST use `monkeypatch.setenv` / `monkeypatch.delenv` to prevent env-var leakage between tests; all backend-switching goes through this fixture with teardown reset.
- **Memory measurement variance**: Python GC behavior makes peak RSS noisy. Solution: use `tracemalloc` (snapshot before/after construction) instead of `resource.getrusage`; record allocated bytes, not RSS. Run three trials and report median; variance must be < 5% across runs per NFR-003.

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: System MUST provide an internal half-edge (DCEL) topology backend selectable via either a constructor argument (`topology_backend='halfedge'`) or the environment variable `CHILMESH_TOPOLOGY_BACKEND`. Specifically: DCEL stored as `ndarray[n_halfedges, 4]` with columns `[origin_vertex, twin_idx, next_idx, face_idx]`, integer-index pointers throughout (no per-half-edge Python objects), boundary half-edges marked `twin_idx == -1`.
...
- **FR-003**: System MUST produce bit-identical adjacency outputs (`Edge2Vert`, `Elem2Edge`, `Vert2Edge`, `Vert2Elem`, `Edge2Elem`, `EdgeMap.to_list()`) between the EdgeMap backend and the half-edge backend on every built-in fixture. Bit-identical defined via canonical-form comparator (edges sorted by `(min_vert, max_vert)`, per-row sets of IDs compared after re-indexing); naive `np.array_equal` is not the contract.
...
- **FR-006**: System MUST emit a markdown comparison table (current EdgeMap vs. half-edge v1 vs. optimized variant) for at least four operations: fast init, full init, quality analysis, query latency. Optimized variant defined as half-edge v1 with Python for-loop replaced by NumPy vectorized `np.take()` scatter/gather on the `next_idx` column; no Numba/Cython/Rust compilation.
...
- **A-003**: The WNAT_Hagen mesh remains available via ADMESH-Domains; if the upstream registry moves the file, the benchmark script's fallback paths catch it. SHA-256 of the file is recorded in the benchmark JSON (`output/benchmark.json`) on the first run and verified on every subsequent run; loud failure if the hash mismatches.
...
- Compiled optimizations of half-edge v1 (Numba JIT, Cython, C extensions) — these are separate Phase 6+ candidates; this phase measures pure-Python baseline only.
