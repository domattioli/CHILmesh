...
├── test_skeletonization_admesh_domains_benchmark.py  # NEW: opt-in MATLAB-parity benchmark on ADMESH-Domains catalog meshes
...
5. **Capture new layer counts**: Run tests, observe new `mesh.n_layers` values, add `tests/test_skeletonization_matlab_parity.py` pinning these values for bundled fixtures, and `tests/test_skeletonization_admesh_domains_benchmark.py` for opt-in parity verification on the ADMESH-Domains catalog.
